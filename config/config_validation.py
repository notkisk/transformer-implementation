...
def validate_config(config: Dict[str, Any]) -> List[str]:
    ...
    if config["d_model"] % 8 != 0:
        warnings_list.append(f"d_model ({config['d_model']}) should be divisible by number of attention heads (recommended: 8)")
    
    # Validate number of layers
    if config.get("num_layers", 6) != 6:
        warnings_list.append(f"paper uses 6 layers, but config specifies {config.get('num_layers', 6)}")
    
    # Validate attention heads
    num_heads = config.get("num_heads", 8)
    if config["d_model"] % num_heads != 0:
        raise ConfigError(f"d_model ({config['d_model']}) must be divisible by number of heads ({num_heads})")
    
    # Validate sequence length
    if config["seq_len"] > 512:
        warnings_list.append(f"positional encodings may not work well for sequences longer than 512 (current: {config['seq_len']})")
    
    # Validate learning rate schedule parameters
    if config.get("warmup_steps", 4000) != 4000:
        warnings_list.append(f"paper uses 4000 warmup steps (current: {config.get('warmup_steps', 4000)})")
    
    # Validate dropout rate
    if config.get("dropout", 0.1) != 0.1:
        warnings_list.append(f"paper uses 0.1 dropout rate (current: {config.get('dropout', 0.1)})")
