...
class EncoderBlock(nn.Module):
    def __init__(self, mha: MultiHeadAttentionBlock, ffn: FeedForward, dropout: float):
        ...
        self.mha = mha
        self.ffn = ffn
        self.residual_blocks = nn.ModuleList([ResidualAddNorm(dropout) for _ in range(2)]) # Two residual add norm blocks, one for mha and one for ffn
        self.dropout = nn.Dropout(dropout)
...
class DecoderBlock(nn.Module):
    def __init__(self, self_mha: MultiHeadAttentionBlock, cross_mha: MultiHeadAttentionBlock, ffn: FeedForward, dropout: float):
        ...
        self.ffn = ffn
        self.residual_blocks = nn.ModuleList([ResidualAddNorm(dropout) for _ in range(3)]) # Three residual connection blocks, one for self attention, one for cross attention and one for feedforward
        self.dropout = nn.Dropout(dropout)
