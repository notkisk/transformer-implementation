...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = x.mean(
            dim=-1, keepdim=True
        )  # dim = -1 becayse: since we are calculating the layer norm, we do it on the token embedding, which is the last dim from x(batch, seq_len, d_model)
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        self.register_buffer(
            "pe", pe.unsqueeze(0)
        )  # shape (1, seq_len, d_model), we unsqueeze because later, this will take input of size (batch_size, sequence_length, d_model), to account for that we add a new dimension to avoid broadcasting error
...
class ResidualAddNorm(nn.Module):
    ...
    def forward(
        self, x, sublayer
    ):  # x is the input to the sublayer, sublayer is a function like the feedforward or attention.
...
class EncoderBlock(nn.Module):
    def __init__(self, mha: MultiHeadAttentionBlock, ffn: FeedForward, dropout: float):
        ...
        self.residual_blocks = nn.ModuleList(
            [ResidualAddNorm(mha.d_model, dropout) for _ in range(2)]
        )  # Two residual add norm blocks, one for mha and one for ffn
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, src_mask):
        x = self.residual_blocks[0](
            x, lambda x: self.mha(x, x, x, src_mask)
...
class DecoderBlock(nn.Module):
    def __init__(
        self,
        self_mha: MultiHeadAttentionBlock,
        ...
        self.ffn = ffn
        self.residual_blocks = nn.ModuleList(
            [ResidualAddNorm(self_mha.d_model, dropout) for _ in range(3)]
        )  # Three residual connection blocks, one for self attention, one for cross attention and one for feedforward
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, enc_out, src_mask, trg_mask):
        x = self.residual_blocks[0](x, lambda x: self.self_mha(x, x, x, trg_mask))
        x = self.residual_blocks[1](
            x, lambda x: self.cross_mha(x, enc_out, enc_out, src_mask)
        )
